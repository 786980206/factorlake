...
- [x] **CREATE TABLE 标准 DDL + 分区创建 + 列组扩展完成（2026-08）**：
  ...
  - 实现：src/catalog/aligned_create.cpp（CreateParquetWriter helper + WriteEmptyParquet + WriteNullParquet + AlignedCreateTable + AlignedCreatePartition + ValidateGroupName）；aligned_catalog.cpp 的 CreateTable 从 NotImplemented 改为调用建表/建分区/扩展（选项解析用 strip_quotes helper）；SupportsCreateTable override 允许 WITH clause（v1.5.4 默认拒绝）；LookupEntry 改为 not-found 返回 nullptr（原 throw 阻止 CREATE TABLE）；EnsureTablesLoaded 允许空 root（原 throw 阻止首次 ATTACH）+ 重加载前 `tables.clear()` 清除旧 schema 缓存（否则扩展后的表仍看到旧列集）
  - **代码审查优化**（2026-08）：提取 `CreateParquetWriter` helper 消除 30 行重复（M1）；extend 模式加 per-column "unknown column" 校验（C2 修复：原先 `col_names.empty()` 只检查零匹配，部分列缺失会静默丢弃）；`AlignedCreatePartition` 改为只检查 index 组（C1 修复：原先首个 group 命中即 throw，错误信息误导）；提取 `strip_quotes` helper 消除 3× 重复代码（C3）；`ValidateGroupName` 在 `ParseGroupsOption` 阶段校验 `lv1/lv2` 格式（M6：原先延后到 BuildTablePlan 才报错，已写入坏目录）；`table_exists` glob 过滤 `_tmp/`（m6：crash 残留 staging 误判为已存在表）；`col_index` map 使列查找 O(1)（M5：原先 O(cols×groups×group_cols) 三重循环）；移除 extend 模式死代码 `template_str`（M3）；`#undef` 移出函数体（m8）
  - 验收：SQLLogicTest 122/122（新增 25 测试：14 建表/分区 + 11 列组扩展含错误用例）；test_aligned 42/42、test_upsert 50/50、test_dml 7/7、test_compaction 16/16、test_parallel 8/8 全 PASS；README.md + docs/STORAGE_CONTRACT.md 同步更新 DDL 章节
