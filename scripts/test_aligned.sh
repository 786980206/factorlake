...
out=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SELECT rowid, ma5 FROM aligned_scan('cnstk_ixday') WHERE rowid IN (0, 1999, 2000, 5999) ORDER BY rowid;")
rows=$(printf '%s\n' "$out" | grep -E '^[0-9]' | tr '\n' ';')
...
out=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SELECT count(*), sum(rowid_alpha) FROM aligned_scan('cnstk_ixday') WHERE rowid >= 2000;")
...
# --- schema evolution --------------------------------------------------------
out=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SELECT alpha099 FROM aligned_scan('cnstk_ixday') WHERE rowid = 0;")
...
out=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SELECT alpha099 FROM aligned_scan('cnstk_ixday') WHERE rowid = 4500;")
...
out=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SELECT symbol FROM aligned_scan('cnstk_ixday') WHERE rowid IN (1999, 2000, 3999, 4000) ORDER BY rowid;")
...
out=$(run_duckdb "SELECT count(*) FROM aligned_scan('cnstk_ixday', root => '$DATA_ROOT');")
if printf '%s' "$out" | grep -qE '^6000$'; then echo 'PASS: aligned_scan(root => ...) variant'; else echo "FAIL: aligned_scan variant ($out)"; failures=$((failures+1)); fi

# --- aggregates / repeat queries ---------------------------------------------
out=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SELECT max(close), count(*) FROM aligned_scan('cnstk_ixday'); SELECT count(*) FROM aligned_scan('cnstk_ixday');")
counts=$(printf '%s\n' "$out" | grep -cE '^6000$')
...
out=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SELECT alpha001, ma20, date FROM aligned_scan('cnstk_ixday') WHERE rowid IN (2000, 4095) ORDER BY rowid;")
...
out=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SELECT count(rowid_ma) FROM aligned_scan('cnstk_ixday');")
...
out=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SELECT count(*) FROM aligned_scan('cnstk_ixday') WHERE date = DATE '2026-08-01';")
...
out=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SELECT count(*) FROM aligned_scan('cnstk_ixday') WHERE date = DATE '2026-07-01';")
...
out=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SELECT count(*) FROM aligned_scan('cnstk_ixday') WHERE date = DATE '2026-08-01' AND rowid < 100;")
...
out=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SELECT count(*) FROM aligned_scan('cnstk_ixday') WHERE date = DATE '2026-08-01' AND rowid >= 2000;")
...
out1=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SET threads=1; SELECT count(*), count(alpha001), count(alpha099), count(rowid_ma), sum(rowid) FROM aligned_scan('cnstk_ixday');")
out8=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SET threads=8; SELECT count(*), count(alpha001), count(alpha099), count(rowid_ma), sum(rowid) FROM aligned_scan('cnstk_ixday');")
...
out1=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SET threads=1; SELECT count(*) FROM aligned_scan('cnstk_ixday') WHERE rowid BETWEEN 3000 AND 3010;")
out8=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SET threads=8; SELECT count(*) FROM aligned_scan('cnstk_ixday') WHERE rowid BETWEEN 3000 AND 3010;")
...
out8=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SET threads=8; SELECT count(rowid_ma), count(alpha099) FROM aligned_scan('cnstk_ixday');")
...
if run_duckdb_expect_error "SET aligned_data_root='$DATA_ROOT'; SELECT \"factor.alpha101.close\" FROM aligned_scan('cnstk_ixday') LIMIT 1;" "not found"; then
...
if run_duckdb_expect_error "SET aligned_data_root='$DATA_ROOT'; SELECT vwap FROM aligned_scan('cnstk_ixday') LIMIT 1;" "not found"; then
...
out=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SELECT \"factor.alpha101.vwap\" AS a, \"fieldset.ma.vwap\" AS m FROM aligned_scan('cnstk_ixday') WHERE rowid = 2000;")
...
if run_duckdb_expect_error "SET aligned_data_root='$DATA_ROOT'; SELECT * FROM aligned_scan('badlvl');" "two-level path"; then
...
if run_duckdb_expect_error "SET aligned_data_root='$DATA_ROOT'; SELECT * FROM aligned_scan('badname');" "self-desc"; then
...
if run_duckdb_expect_error "SET aligned_data_root='$DATA_ROOT'; SELECT * FROM aligned_scan('badidx');" "consecutive from 0000"; then
...
if run_duckdb_expect_error "SET aligned_data_root='$DATA_ROOT'; SELECT * FROM aligned_scan('badrows');" "must agree"; then
...
if run_duckdb_expect_error "SET aligned_data_root='$DATA_ROOT'; SELECT * FROM aligned_scan('baddate');" "second column must be DATE"; then
...
out=$(run_duckdb "SET aligned_data_root='$DATA_ROOT'; SELECT count(*), sum(rowid) FROM aligned_scan('ts_ixday') WHERE ts >= TIMESTAMP '2026-08-02';")
...
if run_duckdb_expect_error "SET aligned_data_root='$DATA_ROOT'; SELECT * FROM aligned_scan('no_such_table');" "does not exist"; then
